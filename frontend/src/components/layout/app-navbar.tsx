...
import { Bell, Search, User, LogOut, Command, HelpCircle } from "lucide-react"
  ...
  return (
    <>
      <header className="sticky top-0 z-30 flex h-12 items-center gap-2 border-b border-border/40 bg-background/80 backdrop-blur-xl px-4 lg:px-6">
        ...
        <div className="flex items-center gap-0.5 ml-1">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-56 rounded-xl border border-border/60 shadow-lg" align="end" sideOffset={8}>
              <DropdownMenuLabel className="font-normal p-3">
                <div className="flex flex-col gap-1">
                  ...
                  <p className="text-xs leading-none text-muted-foreground mt-0.5">{email}</p>
                </div>
              </DropdownMenuLabel>
              <DropdownMenuSeparator />
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem className="h-9 px-3 rounded-lg text-[13px] text-destructive focus:text-destructive hover:bg-destructive/10 cursor-pointer" onClick={() => setIsLogoutOpen(true)}>
      ...
      <Dialog open={isLogoutOpen} onOpenChange={setIsLogoutOpen}>
        <DialogContent className="max-w-sm rounded-xl p-6 bg-background border border-border/60">
